...
files.forEach(file => {
    ...
    // Replace everything from <!-- Sidebar Navigation (Desktop) --> up to JUST BEFORE <nav aria-label="breadcrumb" class="breadcrumbs">
    // Some files might have missing <!-- Sidebar Navigation (Desktop) -->, let's just match from <aside class="sidebar"
    // Wait, some files might have lost the aside entirely, so let's match from <div class="container"> to <nav aria-label="breadcrumb"
    
    const regex = /<div class="container">\s*([\s\S]*?)(?:<!-- Breadcrumbs -->\s*)?(?=<nav aria-label="breadcrumb" class="breadcrumbs">)/g;
    
    if (regex.test(content)) {
        content = content.replace(regex, `<div class="container">\n\n${perfectSidebarAndHeader}`);
